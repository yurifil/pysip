...
from pysip.message.hnames import FROM_HEADER, TO_HEADER, CALLID_HEADER, CSEQ_HEADER, MAXFORWARDS_HEADER
...
class SipMsg(object):
    def __init__(self, message=None):
        if message is None:
            message = Message()
        if message is not None and not isinstance(message, (Message, Request, Response)):
    ...
    def _get_raw(self, hdr):
        return self.__get_header(hdr, ALL)
